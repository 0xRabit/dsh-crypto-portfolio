...
- **资产健康度。** 一个面板用三个指标给组合打分——链上（非 BTC）占比、单一钱包集中度、冷存储占比——每个指标配一条构成条和一句人话建议。阈值写在 `tracker/health.py`（60/70 %、50/75 %、50/20 %），数据取自同一份已过滤黑名单的快照，因此永远和上面的总额一致。在设置里把钱包标成「冷钱包」，托管安全那项**立刻**跟着变（冷热以当前钱包列表为准，快照里的值只作为已删除钱包的兜底）。
